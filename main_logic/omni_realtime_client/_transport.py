...
class _TransportMixin:
    ...
    async def handle_messages(self) -> None:
        ...
        try:
            ...
            provider_session = self.ws
            connection_generation = self._connection_generation

            def receive_owner_is_current() -> bool:
                return bool(
                    connection_generation == self._connection_generation
                    and provider_session is self.ws
                )
            ...
            async for message in provider_session:
            ...
            await self._finish_raw_receive_loop(
            ...
            error_msg = str(e)
            owns_connection = await self._finish_raw_receive_loop(
                error_msg,
                provider_session=provider_session,
                connection_generation=connection_generation,
            )
            logger.error(f"Connection closed with error: {error_msg}")
            if owns_connection and self.on_connection_error:
                await self.on_connection_error(error_msg)
        except asyncio.TimeoutError:
            owns_connection = await self._finish_raw_receive_loop(
    ...
    async def _finish_raw_receive_loop(
        self,
        reason: str,
        *,
        provider_session,
        ...
        """Close only the raw transport owned by the finishing receive loop."""
        ...
        if owns_connection:
            ...
            # A replacement is already live. Retire only the socket captured
            # by this loop; shared fatal/arbiter state belongs to the successor.
            await self._abort_failed_transport(
                reason,
                provider_session,
                connection_generation,

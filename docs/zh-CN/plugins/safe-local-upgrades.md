...
替换或删除用户覆盖版，以及接管旧式“没有 manifest、只有状态目录”的目标时，顶层 `config/`、`data/`、`cache/` 会受到保留保护。为兼容历史插件，N.E.K.O. 会在宿主的 `plugin-installations.json` 中记录这些目录里随包文件的路径和哈希：未被用户修改的作者文件随新包更新或删除，插件运行后新增的文件继续保留；如果用户手改过作者文件，而新包也要写同一路径，安装会以 `PLUGIN_PACKAGE_STATE_CONFLICT` 停止并恢复旧版。新的插件仍建议把只读素材放进 `resources/` 等明确由包拥有的目录，把 `data/` 留给可变运行数据。

...
- `config/`、`data/`、`cache/` 在替换、删除用户覆盖版和兼容旧式“仅状态目录”的安装路径中会受到保留保护。宿主会在 `plugin-installations.json` 记录随包文件的路径和哈希：未被用户修改的作者文件可以随新版更新，运行时新增文件会保留，用户手改作者文件后再升级则会明确报冲突并恢复旧版。旧插件包不需要重新打包；新插件仍建议把只读素材放在 `resources/` 等明确由包拥有的目录，把 `data/` 留给可变运行数据。

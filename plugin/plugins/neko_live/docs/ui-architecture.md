...
| 开发者工具 | `dev` | 调试 | **仅开发者模式开启时出现**；内部固定分为「身份与头像查询 / 模拟直播事件 / 运行结果」三个子页。链路解释、最近 pipeline 结果、高级安全状态、审计摘要、模块总览、沙盒结果和支持事件账本（`get_room_summary` + `query_entries`）统一放在“运行结果”，不占用普通设置页 | — |
...
以下后端能力已定义接口，UI 层暂不实现（等待产品验证后升级到普通面板）：

| 接口 | 触发条件 | 预期展示位置 | 状态 |
|------|---------|-------------|------|
| `LiveSupportEventsModule.get_ledger_summary()` | 开发者模式 | 开发者工具 → 运行结果 | 后端已设计（`support_event_ledger.md`），UI 暂不渲染 |
| `LiveSupportEventsModule.get_ledger_entries()` | 开发者模式 + 明细请求 | 开发者工具 → 运行结果（可折叠表格） | 同上 |
| `query_support_ledger` plugin_entry | 开发者模式 | 聊天工具调用 | 后端 entry 已预留，UI 入口暂不加 |

**升级到普通面板的门槛**：至少 3 位主播在开发者模式下验证账本数据有用，并提出明确的普通面板需求。

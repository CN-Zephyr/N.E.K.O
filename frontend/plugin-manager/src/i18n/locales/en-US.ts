...
export default {
  ...
  market: {
    ...
    install: 'Install',
    installed: 'Installed',
    marketVersion: 'Market v{version}',
    marketInstalledVersion: 'Market v{version}',
    localInstalledVersion: 'Local v{version} · {source}',
    useMarketVersion: 'Use Market version',
    localVersionNewer: 'Local version is newer',
    ...
    externalInstallConfirmationTitle: 'Confirm Market install',
    externalInstallConfirmationBody: 'The Plugin Market requested {mode} for {id} v{version}. Channel: {channel}. Download host: {host}. SHA256: {sha256}. These details came from the requesting page; N.E.K.O will verify the downloaded package before changing any plugin files. Continue?',
  ...
  plugins: {
    ...
    runtimeVersion: {
      ...
      description: 'More than one code installation is available. Choose which one N.E.K.O runs.',
      current: 'In use',
      useThisVersion: 'Use this version',
      builtinHint: 'Built-in code shipped with N.E.K.O',
      marketHint: 'Code installed from the plugin market',
      importedHint: 'Code imported from a local package',
      localHint: 'Local or legacy plugin code',
      sharedDataHint: 'Switching changes only the running code. Config, data, and cache remain shared by the same plugin ID.',
      confirmTitle: 'Switch runtime version',
      confirmBody: 'Use {source} v{version}. A running plugin will stop and restart with this selection; user data will not be deleted.',
      confirmDowngradeBody: 'This switches from v{currentVersion} to the older {source} v{version}. Older code may not understand newer user data. Continue?',
      confirmAction: 'Switch version',
      switchSucceeded: 'Plugin runtime version switched',
      switchFailed: 'Switch failed. N.E.K.O attempted to restore the previous runtime version.',
      unavailable: 'Runtime version switching is unavailable',
      unavailableHint: 'Plugin source records are incomplete or conflicting. Refresh and try again.',
      blockedReason: 'Cannot select: {reason}',
    },
    ...
    batchDeleteConfirm: 'Remove or hide {count} selected plugins? User-installed code will be removed, built-in code will remain available, and plugin data will be kept.',
    ...
    importSuccessRestartRequired: 'Imported {name}. Restart N.E.K.O to switch the running plugin to the newly installed version.',
    ...
    dangerDialog: {
      ...
      deleteMessage: 'Deleting "{pluginName}" removes it from the app. Its settings and data are kept for a future reinstall.',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: 'This folder is not a managed installation and may contain local developer edits. Replacing {current} with {target} will overwrite its code, dependencies, and package assets. The operation will stop if any file changes after this confirmation.',
      upgradeConfirm: 'Upgrade plugin',
      upgradeCancelled: 'Upgrade cancelled.',
      upgradeSucceeded: '{plugin} was upgraded successfully.',
      restartRequired: 'Installation complete. Restart N.E.K.O to use the new version.',
      activationBlocked: 'The plugin was not activated, so installation was not reported as successful. Reason: {reason}',
      ...
      error: {
        ...
        manifestInvalid: 'The package manifest.toml is invalid. Ask the plugin author to rebuild the package.',
        ...
        packageTypeMismatch: 'The declared package type does not match the plugins it contains. Ask the plugin author to rebuild it.',
        hashMismatch: 'The package contents do not match its verification data. Nothing was installed.',
        hashMismatchRolledBack: 'The package contents do not match its verification data. This import was rejected and the pre-install state was restored.',
        packageStateConflict: 'Plugin data you edited conflicts with the new version. The current plugin was restored; back up the edited file, then retry.',
  ...
  messages: {
    ...
    pluginRevertedToBuiltin: 'User-installed version removed; switched back to the built-in version',

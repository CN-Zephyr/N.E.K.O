...
export default {
  ...
  market: {
    ...
    install: '설치',
    installed: '설치됨',
    marketVersion: '마켓 v{version}',
    marketInstalledVersion: '마켓 버전 v{version}',
    localInstalledVersion: '로컬 v{version} · {source}',
    useMarketVersion: '마켓 버전 사용',
    localVersionNewer: '로컬 버전이 더 최신임',
    installing: '설치 중...',
    installSuccess: '설치 작업 생성: {name}',
    installFailed: '설치 실패',
    installPreparing: '설치 준비 중...',
    installAlreadyRunning: '다른 설치가 진행 중입니다. 완료될 때까지 기다려 주세요.',
    externalInstallConfirmationTitle: '마켓 설치 확인',
    externalInstallConfirmationBody: '플러그인 마켓에서 {id} v{version}의 {mode} 작업을 요청했습니다. 채널: {channel}. 다운로드 호스트: {host}. SHA256: {sha256}. 이 정보는 요청 페이지가 제공한 값이며 N.E.K.O는 파일 변경 전에 패키지를 다시 검증합니다. 계속할까요?',
    cancelInstall: '설치 취소',
  ...
  plugins: {
    ...
    runtimeVersion: {
      title: '실행 버전',
      description: '여러 코드 설치본이 있습니다. 실제로 실행할 버전을 선택하세요.',
      current: '사용 중',
      useThisVersion: '이 버전 사용',
      builtinHint: 'N.E.K.O에 포함된 기본 코드',
      marketHint: '플러그인 마켓에서 설치한 코드',
      importedHint: '로컬 패키지에서 가져온 코드',
      localHint: '로컬 또는 레거시 플러그인 코드',
      sharedDataHint: '전환은 실행 코드만 변경합니다. 같은 플러그인 ID의 설정, 데이터, 캐시는 계속 공유됩니다.',
      confirmTitle: '실행 버전 전환',
      confirmBody: '{source} v{version}(으)로 전환합니다. 실행 중인 플러그인은 중지한 뒤 새 선택으로 다시 시작하며 사용자 데이터는 삭제하지 않습니다.',
      confirmDowngradeBody: 'v{currentVersion}에서 이전 {source} v{version}(으)로 전환합니다. 이전 코드가 최신 사용자 데이터를 읽지 못할 수 있습니다. 계속할까요?',
      confirmAction: '버전 전환',
      switchSucceeded: '플러그인 실행 버전을 전환했습니다',
      switchFailed: '전환에 실패했습니다. 이전 실행 버전 복원을 시도했습니다.',
      unavailable: '실행 버전을 전환할 수 없습니다',
      unavailableHint: '플러그인 출처 기록이 불완전하거나 충돌합니다. 새로 고친 뒤 다시 시도하세요.',
      blockedReason: '선택할 수 없음: {reason}',
    },
    ...
    batchDeleteConfirm: '선택한 {count}개의 플러그인을 제거하거나 숨길까요? 사용자가 설치한 코드는 제거되며 기본 제공 코드와 플러그인 데이터는 유지됩니다.',
    ...
    importSuccessRestartRequired: '{name}을(를) 가져왔습니다. 실행 중인 플러그인을 새 버전으로 전환하려면 N.E.K.O를 다시 시작하세요.',
    ...
    dangerDialog: {
      ...
      deleteMessage: '"{pluginName}"을 삭제하면 앱에서 제거됩니다. 나중에 다시 설치할 수 있도록 설정과 데이터는 유지됩니다.',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: '이 폴더는 플러그인 센터가 관리하는 설치가 아니며 개발자의 로컬 수정 사항이 있을 수 있습니다. 계속하면 {current}을(를) {target}(으)로 교체하면서 코드, 종속성 및 패키지 자산을 덮어씁니다. 확인 후 파일이 변경되면 설치가 중단됩니다.',
      upgradeConfirm: '플러그인 업그레이드',
      upgradeCancelled: '업그레이드를 취소했습니다.',
      upgradeSucceeded: '{plugin}이(가) 성공적으로 업그레이드되었습니다.',
      restartRequired: '설치가 완료되었습니다. 새 버전을 사용하려면 N.E.K.O를 다시 시작하세요.',
      activationBlocked: '플러그인을 안전하게 활성화하지 못했으므로 설치 성공으로 처리하지 않았습니다. 이유: {reason}',
      ...
      error: {
        ...
        manifestInvalid: '패키지 manifest.toml 형식이 잘못되었습니다. 플러그인 작성자에게 수정 후 다시 빌드하도록 요청하세요.',
        ...
        packageTypeMismatch: '선언된 패키지 유형과 포함된 플러그인 수가 일치하지 않습니다. 작성자에게 다시 빌드하도록 요청하세요.',
        hashMismatch: '패키지 내용이 검증 정보와 일치하지 않아 설치를 거부했습니다. 설치 변경은 남지 않았습니다.',
        hashMismatchRolledBack: '패키지 내용이 검증 정보와 일치하지 않아 가져오기를 거부하고 설치 전 상태를 복원했습니다.',
        packageStateConflict: '수정한 플러그인 데이터가 새 버전과 충돌합니다. 현재 플러그인을 복원했습니다. 수정한 파일을 백업한 후 다시 시도하세요.',

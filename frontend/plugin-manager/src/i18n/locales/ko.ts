...
export default {
  ...
  plugins: {
    ...
    batchDeleteConfirm: '선택한 {count}개의 플러그인을 삭제하시겠습니까? 플러그인 코드 디렉터리는 제거되지만 관련 패키지 설정과 사용자 데이터는 유지됩니다.',
    ...
    dangerDialog: {
      ...
      deleteMessage: '"{pluginName}"을 삭제하면 플러그인 코드 디렉터리가 제거됩니다. 관련 패키지 설정과 사용자 데이터는 유지됩니다. 목록도 즉시 새로고침됩니다.',
    ...
    retainedData: {
      entry: '보존 데이터',
      title: '제거 후 보존된 데이터',
      description: '플러그인 코드가 제거된 뒤에도 남아 있는 패키지 설정 데이터를 관리합니다.',
      scopeNotice: '패키지가 명시적으로 생성하고 소유한 설정 디렉터리만 삭제합니다. 다른 플러그인 상태나 임의의 사용자 파일은 삭제하지 않습니다.',
      empty: '관리할 보존 데이터가 없습니다',
      packageId: '패키지',
      candidate: '이전 후보',
      codePresent: '후보 코드가 아직 있습니다. 먼저 일반 플러그인 제거를 완료하세요.',
      deleteAction: '보존 데이터 삭제',
      confirmTitle: '보존 데이터 삭제 확인',
      confirmMessage: '플러그인 "{pluginId}"에 대해 패키지가 만든 설정 데이터를 영구 삭제합니다. 플러그인 코드는 변경되지 않습니다.',
      confirmHint: '삭제 전에 소유권, 공유 여부, 경로 안전을 다시 확인합니다. 실수를 막으려면 아래 버튼을 길게 누르세요.',
      warningTitle: '이 데이터는 복구할 수 없습니다',
      holdIdle: '길게 눌러 보존 데이터 삭제',
      holdActive: '계속 눌러 확인하세요…',
      deleting: '보존 데이터를 삭제하는 중…',
      loadFailed: '보존 데이터를 불러오지 못했습니다',
      deleteSuccess: '보존 데이터를 삭제했습니다',
      deleteFailed: '보존 데이터를 삭제하지 못했습니다',
    },
    ui: {
      open: 'UI 열기',
      title: 'UI',
  ...
  logs: {
    ...
    clear: '로그 지우기',
    clearTitle: '플러그인 로그 지우기',
    clearConfirm: '이 플러그인의 저장된 로그를 모두 영구적으로 지웁니다. 플러그인이 실행 중이면 새 로그가 계속 생성될 수 있습니다.',
    clearSuccess: '플러그인 로그를 지웠습니다',
    clearFailed: '플러그인 로그를 지우지 못했습니다'
  },
  runs: {
    title: '실행 기록',
    detail: '실행 상세',

...
export default {
  ...
  market: {
    ...
    install: '安装',
    installed: '已安装',
    marketVersion: '市场 v{version}',
    marketInstalledVersion: '市场版 v{version}',
    localInstalledVersion: '本地 v{version} · {source}',
    useMarketVersion: '改用市场版',
    localVersionNewer: '本地版本较新',
    installing: '安装中...',
    installSuccess: '安装完成: {name}',
    installFailed: '安装失败',
    installPreparing: '正在准备安装...',
    installAlreadyRunning: '已有安装任务正在进行，请等待其完成',
    externalInstallConfirmationTitle: '确认市场安装',
    externalInstallConfirmationBody: '插件市场请求对 {id} v{version} 执行 {mode}。渠道：{channel}；下载域名：{host}；SHA256：{sha256}。这些信息由发起请求的网页提供，N.E.K.O 会在改动插件文件前再次校验下载包。是否继续？',
    cancelInstall: '取消安装',
  ...
  plugins: {
    ...
    runtimeVersion: {
      title: '运行版本',
      description: '这个插件有多份代码可用，你可以选择实际运行哪一份。',
      current: '当前使用',
      useThisVersion: '使用这一版',
      builtinHint: '随 N.E.K.O 提供的内置代码',
      marketHint: '从插件市场安装的代码',
      importedHint: '从本地安装包导入的代码',
      localHint: '本地或旧版插件代码',
      sharedDataHint: '切换只改变运行代码；配置、数据和缓存仍由同一插件 ID 共用。',
      confirmTitle: '切换运行版本',
      confirmBody: '将改用{source} v{version}。运行中的插件会先停止，再使用新选择重新启动；用户数据不会被删除。',
      confirmDowngradeBody: '这会从 v{currentVersion} 切换到较旧的{source} v{version}。旧代码可能无法读取较新的用户数据，确认仍要切换吗？',
      confirmAction: '确认切换',
      switchSucceeded: '已切换插件运行版本',
      switchFailed: '切换失败，已尽量恢复原来的运行版本',
      unavailable: '暂时无法切换运行版本',
      unavailableHint: '插件来源记录不完整或存在冲突，请刷新后再试。',
      blockedReason: '无法选择：{reason}',
    },
    ...
    batchDeleteConfirm: '确认移除或隐藏 {count} 个插件？用户安装的代码会被删除，内置代码仍会保留，插件数据不会清除。',
    ...
    importSuccessRestartRequired: '已导入 {name}。请重启 N.E.K.O，将正在运行的插件切换到新安装的版本。',
    importFailed: '导入失败',
    export: '导出',
    ...
    dangerDialog: {
      title: '危险操作确认',
      warningTitle: '不可逆操作',
      deleteMessage: '删除插件“{pluginName}”后，它会从应用中移除；设置和数据将保留，方便以后重新安装。',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: '这个目录不是由插件中心管理的安装，里面可能有开发者手改内容。继续会用 {target} 替换 {current}，覆盖代码、依赖和包内资源；确认后只要任一文件发生变化，安装就会停止。',
      upgradeConfirm: '升级插件',
      upgradeCancelled: '已取消升级。',
      upgradeSucceeded: '{plugin} 已成功升级。',
      restartRequired: '安装完成。重启 N.E.K.O 后将使用新版本。',
      activationBlocked: '插件没有安全激活，因此本次不算安装成功。原因：{reason}',
      ...
      error: {
        ...
        manifestInvalid: '安装包的 manifest.toml 格式有误，请让插件作者修复后重新打包。',
        ...
        packageTypeMismatch: '安装包声明的类型与其中的插件数量不一致，请让插件作者重新打包。',
        hashMismatch: '安装包内容与校验信息不一致，已拒绝安装，未保留任何安装改动。',
        hashMismatchRolledBack: '安装包内容与校验信息不一致，已拒绝此次导入并恢复安装前状态。',
        packageStateConflict: '你修改过的插件数据与新版本冲突。当前插件已恢复；请先备份修改过的文件，再重试。',

...
export default {
  ...
  plugins: {
    ...
    batchDeleteConfirm: '确认批量删除 {count} 个插件？插件代码目录会被移除，关联配置与用户数据会保留。',
    ...
    dangerDialog: {
      title: '危险操作确认',
      warningTitle: '不可逆操作',
      deleteMessage: '删除插件“{pluginName}”后，其代码目录会被移除，关联配置与用户数据会保留。当前列表会立即刷新。',
    ...
    retainedData: {
      entry: '保留数据',
      title: '卸载后保留的数据',
      description: '管理插件代码删除后仍保留的安装包配置数据。',
      scopeNotice: '这里只删除安装包明确创建且仍由其拥有的配置目录，不会清除其他插件状态或任意用户文件。',
      empty: '没有可管理的保留数据',
      packageId: '数据包',
      candidate: '原候选',
      codePresent: '候选代码仍然存在，请先完成普通插件卸载。',
      deleteAction: '删除保留数据',
      confirmTitle: '确认删除保留数据',
      confirmMessage: '将永久删除插件“{pluginId}”由安装包创建的配置数据。插件代码不会因此改变。',
      confirmHint: '删除前会再次检查数据归属、共享情况和路径安全。为避免误触，请按住下方按钮。',
      warningTitle: '此数据删除后无法恢复',
      holdIdle: '按住以删除保留数据',
      holdActive: '继续按住，正在确认…',
      deleting: '正在删除保留数据…',
      loadFailed: '加载保留数据失败',
      deleteSuccess: '保留数据已删除',
      deleteFailed: '删除保留数据失败',
    },
    ui: {
      open: '打开界面',
      title: '界面',
  ...
  logs: {
    ...
    clear: '清空日志',
    clearTitle: '清空插件日志',
    clearConfirm: '这会永久清空该插件已保存的所有日志。插件运行期间产生的新日志仍可能继续出现。',
    clearSuccess: '插件日志已清空',
    clearFailed: '清空插件日志失败'
  },
  runs: {
    title: '运行记录',
    detail: '运行详情',

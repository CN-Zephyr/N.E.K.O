...
export default {
  ...
  market: {
    ...
    marketVersion: 'Маркет v{version}',
    marketInstalledVersion: 'Версия из маркета v{version}',
    localInstalledVersion: 'Локальная v{version} · {source}',
    useMarketVersion: 'Использовать версию из маркета',
    localVersionNewer: 'Локальная версия новее',
    ...
    externalInstallConfirmationTitle: 'Подтвердите установку из Market',
    externalInstallConfirmationBody: 'Маркет запросил операцию {mode} для {id} v{version}. Канал: {channel}. Узел загрузки: {host}. SHA256: {sha256}. Эти данные предоставлены страницей запроса; N.E.K.O проверит пакет до изменения файлов. Продолжить?',
  ...
  plugins: {
    ...
    runtimeVersion: {
      title: 'Запускаемая версия',
      description: 'Доступно несколько установок кода. Выберите, какую из них будет запускать N.E.K.O.',
      current: 'Используется',
      useThisVersion: 'Использовать эту версию',
      builtinHint: 'Встроенный код, поставляемый с N.E.K.O',
      marketHint: 'Код, установленный из магазина плагинов',
      importedHint: 'Код, импортированный из локального пакета',
      localHint: 'Локальный или устаревший код плагина',
      sharedDataHint: 'Переключение меняет только запускаемый код. Настройки, данные и кэш остаются общими для одного ID плагина.',
      confirmTitle: 'Переключить запускаемую версию',
      confirmBody: 'Будет использоваться {source} v{version}. Работающий плагин остановится и запустится с новым выбором; данные пользователя не удаляются.',
      confirmDowngradeBody: 'Будет выполнен переход с v{currentVersion} на более старую {source} v{version}. Старый код может не прочитать новые данные пользователя. Продолжить?',
      confirmAction: 'Переключить',
      switchSucceeded: 'Запускаемая версия плагина переключена',
      switchFailed: 'Переключение не удалось. N.E.K.O попытался восстановить предыдущую версию.',
      unavailable: 'Переключение версии недоступно',
      unavailableHint: 'Данные об источниках неполны или конфликтуют. Обновите и повторите попытку.',
      blockedReason: 'Нельзя выбрать: {reason}',
    },
    ...
    batchDeleteConfirm: 'Удалить или скрыть {count} выбранных плагинов? Установленный пользователем код будет удален, встроенный код сохранится, а данные плагина останутся.',
    ...
    importSuccessRestartRequired: '{name} импортирован. Перезапустите N.E.K.O, чтобы переключить работающий плагин на новую версию.',
    ...
    dangerDialog: {
      ...
      deleteMessage: 'После удаления "{pluginName}" исчезнет из приложения. Настройки и данные сохранятся для будущей переустановки.',
  ...
  package: {
    install: {
      ...
      upgradeUnmanagedBody: 'Эта папка не является управляемой установкой и может содержать локальные изменения разработчика. Продолжение заменит {current} на {target} и перезапишет код, зависимости и ресурсы пакета. Если после подтверждения изменится любой файл, установка будет остановлена.',
      upgradeConfirm: 'Обновить плагин',
      upgradeCancelled: 'Обновление отменено.',
      upgradeSucceeded: 'Плагин {plugin} успешно обновлён.',
      restartRequired: 'Установка завершена. Перезапустите N.E.K.O, чтобы использовать новую версию.',
      activationBlocked: 'Плагин не удалось безопасно активировать, поэтому установка не считается успешной. Причина: {reason}',
      ...
      error: {
        ...
        manifestInvalid: 'Файл manifest.toml пакета повреждён. Попросите автора исправить и пересобрать пакет.',
        ...
        packageTypeMismatch: 'Заявленный тип пакета не соответствует количеству плагинов внутри. Попросите автора пересобрать пакет.',
        hashMismatch: 'Содержимое пакета не совпадает с данными проверки. Установка отклонена, изменения не сохранены.',
        hashMismatchRolledBack: 'Содержимое пакета не совпадает с данными проверки. Импорт отклонён, состояние до установки восстановлено.',
        packageStateConflict: 'Изменённые вами данные плагина конфликтуют с новой версией. Текущая версия восстановлена; сохраните изменённый файл и повторите попытку.',

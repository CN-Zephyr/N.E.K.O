...
export default {
  ...
  plugins: {
    ...
    candidates: {
      ...
      hint: 'Одновременно работает только одна версия. При переключении плагин безопасно перезапускается.',
      running: 'Запущена',
      current: 'Выбрана',
      invalid: 'Недоступна',
      switch: 'Использовать эту версию',
      none: 'Установленные версии не найдены',
      loadFailed: 'Не удалось загрузить установленные версии',
      switchSuccess: 'Версия плагина переключена',
      switchFailed: 'Переключение не удалось; сохранена предыдущая версия',
      dataAccessRequired: 'Доступ к существующим данным',
      dataAccessTitle: 'Разрешить доступ к существующим данным плагина?',
      dataAccessPrompt: 'Эта версия из источника {source} сможет читать и изменять существующие настройки, данные, кэш и сохранённые учётные данные плагина.',
      dataAccessConfirm: 'Разрешить и переключить',
    },
    installSource: {
      channel: {
        builtin: 'Встроенный',
    ...
    batchDeleteConfirm: 'Удалить {count} выбранных плагинов? Каталоги с кодом плагинов будут удалены, а связанные настройки пакетов и пользовательские данные будут сохранены.',
    ...
    dangerDialog: {
      ...
      deleteMessage: 'Удаление "{pluginName}" удалит каталог с кодом плагина. Связанные настройки пакета и пользовательские данные будут сохранены. Список будет сразу обновлён.',
    ...
    retainedData: {
      entry: 'Сохранённые данные',
      title: 'Данные, сохранённые после удаления',
      description: 'Управление данными конфигурации пакета, оставшимися после удаления кода плагина.',
      scopeNotice: 'Удаляются только каталоги конфигурации, явно созданные пакетом и принадлежащие ему. Прочее состояние плагинов и произвольные файлы пользователя не затрагиваются.',
      empty: 'Нет сохранённых данных для управления',
      packageId: 'Пакет',
      candidate: 'Прежний кандидат',
      codePresent: 'Код кандидата всё ещё существует. Сначала удалите плагин обычным способом.',
      deleteAction: 'Удалить сохранённые данные',
      confirmTitle: 'Подтвердите удаление данных',
      confirmMessage: 'Данные конфигурации, созданные пакетом плагина «{pluginId}», будут удалены безвозвратно. Код плагина не изменится.',
      confirmHint: 'Перед удалением повторно проверяются владелец, совместное использование и безопасность пути. Для защиты от случайного удаления удерживайте кнопку ниже.',
      warningTitle: 'Эти данные нельзя восстановить',
      holdIdle: 'Удерживайте для удаления данных',
      holdActive: 'Продолжайте удерживать…',
      deleting: 'Удаление сохранённых данных…',
      loadFailed: 'Не удалось загрузить сохранённые данные',
      deleteSuccess: 'Сохранённые данные удалены',
  ...
  logs: {
    ...
    clear: 'Очистить логи',
    clearTitle: 'Очистить логи плагина',
    clearConfirm: 'Все сохранённые логи этого плагина будут безвозвратно очищены. Пока плагин работает, новые записи могут появляться снова.',
    clearSuccess: 'Логи плагина очищены',
    clearFailed: 'Не удалось очистить логи плагина'
  },
  runs: {
    title: 'Запуски',
